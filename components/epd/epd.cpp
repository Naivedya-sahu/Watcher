...
void epd_flush(const uint8_t *buf, epd_refresh_t refresh)
{
    ...
    // invert=false  : FB_WHITE=1 bit → white pixel (correct for 1-bit buf)
    // mirror_y=false: fb (0,0) = display top-left; rows sent top→bottom
    // If display appears upside-down physically, flip mirror_y back to true
    // AND invert the Y coordinates in all screen render functions instead.
    if (refresh == EPD_REFRESH_FULL)
    {
        s_display->writeImageForFullRefresh(buf, 0, 0, EPD_W, EPD_H, false, false);
        s_display->refresh(false);
    }
    else
    {
        s_display->writeImage(buf, 0, 0, EPD_W, EPD_H, false, false);
...
void epd_flush_region(const uint8_t *buf, int x, int y, int w, int h)
{
    ...
    s_display->writeImagePart(buf, x, y, EPD_W, EPD_H, x, y, w, h, false, false);
