...
static volatile int      s_btn_a_count = 0;  // BTN_A press count
static volatile int      s_btn_b_count = 0;  // BTN_B press count
static volatile int      s_btn_c_count = 0;  // BTN_C press count
...
static volatile bool s_btn_a_event = false;  // BTN_A pressed
static volatile bool s_btn_b_event = false;  // BTN_B pressed
static volatile bool s_btn_c_event = false;  // BTN_C pressed
...
// ── Button ISRs (active-low, falling edge) ───────────────────────
...
static void IRAM_ATTR btn_c_isr(void *arg) {
    s_btn_c_count++;
    s_btn_c_event = true;
}
...
static void periph_init(void) {
    ...
    // Button A — falling edge (active-low)
    gpio_config_t btn_a = {
    ...
    // Button B — falling edge (active-low)
    gpio_config_t btn_b = {
        .pin_bit_mask = (1ULL << PIN_BTN_B),
        .mode         = GPIO_MODE_INPUT,
        .pull_up_en   = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type    = GPIO_INTR_NEGEDGE,
    ...
    // Button C — falling edge (active-low)
    gpio_config_t btn_c = {
        .pin_bit_mask = (1ULL << PIN_BTN_C),
        .mode         = GPIO_MODE_INPUT,
        .pull_up_en   = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type    = GPIO_INTR_NEGEDGE,
    ...
    gpio_isr_handler_add(PIN_BTN_A,  btn_a_isr, NULL);
    gpio_isr_handler_add(PIN_BTN_B,  btn_b_isr, NULL);
    gpio_isr_handler_add(PIN_BTN_C,  btn_c_isr, NULL);
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "Press buttons (IO40/39/38) → buzzer plays tone, cycles A4/A5/C6/E6/OFF");
    ESP_LOGI(TAG, "======================================================");
    ...
    while (true) {
        ...
        if (s_btn_a_event) {
            ...
            if (freq == 0)
                printf("[BTN_A] press #%d  — buzzer: OFF\n", s_btn_a_count);
            else
                printf("[BTN_A] press #%d  — buzzer: %lu Hz\n",
                       s_btn_a_count, (unsigned long)freq);
        ...
        if (s_btn_b_event) {
            ...
            if (freq == 0)
                printf("[BTN_B] press #%d  — buzzer: OFF\n", s_btn_b_count);
            else
                printf("[BTN_B] press #%d  — buzzer: %lu Hz\n",
                       s_btn_b_count, (unsigned long)freq);
        ...
        if (s_btn_c_event) {
            ...
            if (freq == 0)
                printf("[BTN_C] press #%d  — buzzer: OFF\n", s_btn_c_count);
            else
                printf("[BTN_C] press #%d  — buzzer: %lu Hz\n",
                       s_btn_c_count, (unsigned long)freq);
