...
void drawTitle() {
    char title[20];
    snprintf(title, sizeof(title), "%dmin POMODORO", TIMER_MODES[timerModeIndex]);
    Paint_DrawString_EN(130, 15, title, &Font20, BLACK, WHITE);
}

// Draw button labels at bottom (separate region from timer)
void drawButtons() {
    Paint_DrawString_EN(BTN_START_X, BUTTON_Y, isRunning ? "PAUSE" : "START", &Font16, BLACK, WHITE);
    Paint_DrawString_EN(BTN_MODE_X, BUTTON_Y, "MODE", &Font16, BLACK, WHITE);
    Paint_DrawString_EN(BTN_RESET_X, BUTTON_Y, "RESET", &Font16, BLACK, WHITE);
}
...
void handleMode() {
    ...
    // Full refresh needed because title changes
    drawFullScreen();
...
void checkButtons() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (state != lastBtnState[i] && (now - lastBtnDebounce[i] > 150)) {
            lastBtnDebounce[i] = now;

            if (state == LOW) {  // Button pressed (active low)
                switch (i) {
...
void setup() {
    ...
    lastBtnState[0] = digitalRead(SW1_PIN);
    lastBtnState[1] = digitalRead(SW2_PIN);
    lastBtnState[2] = digitalRead(SW3_PIN);
